...
void
FillEntropy64(uint8_t *entropy) {
    for (int i = 0; i < 8; i++)
        memcpy(g_pui32RandomEntropy + (i * 2), entropy, 8);
}

void
SetEntropyUsing(uint8_t *msg, unsigned long len) {
    memset(g_pui32RandomEntropy, 0, 64);
    memcpy(g_pui32RandomEntropy, msg, len);
}

// uint32_t
// GetEntropyAddress()
// {
//     return ((uint32_t *)g_pui32RandomEntropy);
// }
...
#define H(a, b, c, d, k, s)                                            \
    ...
    memcpy(digest, &ui32A, 4);
    memcpy(digest+4, &ui32B, 4);
    memcpy(digest+8, &ui32C, 4);
    memcpy(digest+12, &ui32D, 4);
}
