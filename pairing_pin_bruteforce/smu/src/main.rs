...
struct Args {
    ...
    /// Delay between PIN attempts in milliseconds. Default = 5.
    #[arg(short, long)]
...
const DEFAULT_PIN_ATTEMPT_DELAY: u32 = 5; // In milliseconds.

fn pair(uart0: &mut Box<dyn SerialPort>, pin: u32) {
    let start_msg = b"pair\n";
    uart0
        .write_all(start_msg)
...
fn main() {
    ...
    for pin in start_pin..=stop_pin {
        ...
        let mut buf = [0u8; 3];
        match uart1.read_exact(&mut buf) {
            Ok(()) => {
                if buf[2] == 1 {
